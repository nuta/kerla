...
impl BitMapAllocator {
    pub fn new(base: *mut u8, base_paddr: usize, len: usize) -> BitMapAllocator {
        ...
        let bitmap = BitSlice::from_slice_mut(unsafe {
            core::slice::from_raw_parts_mut(base, bitmap_actual_len)
        })
    ...
    pub fn free_pages(&mut self, ptr: usize, order: usize) {
        ...
        let mut bitmap = self.bitmap.lock();
        self.bitmap.lock()[off..(off + num_pages)].set_all(false);
    }
}
