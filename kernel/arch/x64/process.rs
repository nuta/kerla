...
impl Process {
    ...
    pub fn new_kthread(ip: VAddr, sp: VAddr) -> Process {
        ...
        let syscall_stack =
            alloc_pages_owned(KERNEL_STACK_SIZE / PAGE_SIZE, AllocPageFlags::KERNEL)
                .expect("failed to allocate kernel stack");
        let kernel_stack = alloc_pages_owned(KERNEL_STACK_SIZE / PAGE_SIZE, AllocPageFlags::KERNEL)
    ...
    pub fn new_user_thread(ip: UserVAddr, sp: UserVAddr) -> Process {
        ...
        let syscall_stack =
            alloc_pages_owned(KERNEL_STACK_SIZE / PAGE_SIZE, AllocPageFlags::KERNEL)
                .expect("failed to allocate kernel stack");
        let xsave_area = alloc_pages_owned(1, AllocPageFlags::KERNEL | AllocPageFlags::ZEROED)
